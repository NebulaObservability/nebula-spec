...
跨语言实现必须消费同一批输入输出用例。`manifest.yaml` 中的协议、ACK、配置和 Receiver Fixture 由 JSON Schema 自动验证；Session、Privacy 和 Trace 关联 Fixture 供 SDK、Agent、Collector 与 Backend 执行行为一致性测试。
